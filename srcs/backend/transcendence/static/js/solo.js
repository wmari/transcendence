...
function startGameSolo() {
	...
	function moveAI() {
		let tolerance = 2; // Small buffer to prevent shaking
	
		if (ballDirX < 0) {
			// If the ball is moving towards player 1, AI centers itself slowly
			if (paddle2.position.z < -fieldHeight / 2 + paddleDepth / 2 + tolerance)
				paddle2.position.z += paddleSpeed / 2;
			else if (paddle2.position.z > fieldHeight / 2 - paddleDepth / 2 - tolerance)
				paddle2.position.z -= paddleSpeed / 2;
		}
	
		if (ballDirX > 0) {
			// AI moves only if ball is outside a small tolerance range
			if (ball.position.z < paddle2.position.z - paddleDepth / 3)
				paddle2.position.z -= paddleSpeed;
			else if (ball.position.z > paddle2.position.z + paddleDepth / 3)
				paddle2.position.z += paddleSpeed;
		}
	
		// Prevent AI paddle from going out of bounds
		if (paddle2.position.z < -fieldHeight / 2 + paddleDepth / 2) {
			paddle2.position.z = -fieldHeight / 2 + paddleDepth / 2;
		} else if (paddle2.position.z > fieldHeight / 2 - paddleDepth / 2) {
			paddle2.position.z = fieldHeight / 2 - paddleDepth / 2;
		}
	}
